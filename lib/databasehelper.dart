...
class DatabaseHelper {
  ...
  initDb() async {
    ...
    String path = join(documentDirectory.path, "doc.db");
    var ourDb = await openDatabase(path, version: 1, onCreate: _onCreate);
    return ourDb;
  }
  void _onCreate(Database db, int version) async {
        await db.execute(
        "CREATE TABLE $tableUser($columnId INTEGER PRIMARY KEY, $columnName TEXT, $columnGrad TEXT, $columnExp TEXT, $columnProfilimage TEXT, $columnCovlimage TEXT, $columnWilaya TEXT, $columnLocation TEXT, $columnEmail TEXT, $columnNumber TEXT, $columnTiming TEXT, $columnReco INTEGER)");
  }
  ...
  Future<int> updateUser(User user) async {
    ...
    return await dbClient.update(tableUser, user.toMap(),
        where: "$columnName = ?", whereArgs: [user.nom]);
  }
  Future close() async {
    var dbClient = await db;
